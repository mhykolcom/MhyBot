        ...
        if (platform == "twitch") {
            ...
            if (youtubeMember) {
                ...
                client.MongoClient.connect(client.MongoUrl, { useNewUrlParser: true }, function (err, db) {
                    if (err) throw err;
                    var dbo = db.db("mhybot")
                    dbo.collection("servers").updateOne({ _id: client.currentserver._id }, { $pull: { youtubeChannels: { name: youtubeMember.name } } }, function (err, res) {
                        if (err) throw err;
                        message.reply("Removed " + streamer + ".");
                        client.logger.info(`[${server.name}] Channel Deleted: ${youtubeMember.name}`)
                    })
                    db.close()
                })
            } else {
